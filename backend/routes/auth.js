...
router.post('/register', async (req, res) => {
  try {
    ...
    if (phone) {
      ...
      if (phoneExists) return res.status(400).json({ message: 'Yeh phone pehle se registered hai. Dusra number use karo.' });
    }
...
router.get('/users', auth, async (req, res) => {
  try {
    const users = await User.find({}, 'name email phone createdAt role').sort({ createdAt: -1 });
    res.json(users);
