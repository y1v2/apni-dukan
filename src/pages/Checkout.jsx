...
const RAZORPAY_KEY = 'rzp_test_YOUR_KEY_HERE'; // ← Yahan apna key daalo

export default function Checkout() {
  const { cartItems, total, savings, clearCart } = useCart();
  const navigate = useNavigate();
  const user = JSON.parse(localStorage.getItem('user') || 'null');
  const token = localStorage.getItem('token');

  const [address, setAddress] = useState({
    name: user?.name || '', phone: '', street: '', city: 'Kalpi',
  ...
  const captureLocation = () => {
    if (!navigator.geolocation) { toast.error('Location support nahi hai is browser mein'); return; }
    navigator.geolocation.getCurrentPosition(
      pos => {
        setAddress(prev => ({ ...prev, location: { lat: pos.coords.latitude, lng: pos.coords.longitude } }));
        setLocationCaptured(true);
        toast.success('Location capture ho gaya! 📍');
      },
      () => toast.error('Location access deny hua — manually address bharo')
    );
  };
  ...
  const validate = () => {
    ...
    if (!address.phone.trim() || address.phone.length < 10) { toast.error('Sahi phone number bharo'); return false; }
  ...
  const placeOrderCOD = async () => {
    ...
    try {
      ...
      toast.success(`Order placed! Receipt: #${data.receiptNumber} 🎉`);
      ...
      window.open(`https://wa.me/919670944301?text=${encodeURIComponent(msg)}`, '_blank');

      clearCart();
      navigate('/orders');
    } catch (e) {
      toast.error(e.response?.data?.message || 'Order place karne mein error aaya');
    }
    setLoading(false);
  };
  ...
  const payOnline = async () => {
    if (!validate()) return;
    if (RAZORPAY_KEY === 'rzp_test_YOUR_KEY_HERE') { toast.error('Razorpay key set nahi hai. COD use karo abhi.'); return; }
    setLoading(true);
    ...
    const options = {
      ...
      handler: async (response) => {
        try {
          ...
          toast.success(`Payment successful! Receipt: #${data.receiptNumber} 🎉`);
          clearCart();
          navigate('/orders');
  ...
  if (cartItems.length === 0) return (
    <div style={{ minHeight:'80vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'#f5f9f6', padding:20 }}>
      ...
      <button onClick={() => navigate('/')} style={{ background:'#145530', color:'#fff', border:'none', borderRadius:50, padding:'14px 36px', fontSize:15, fontWeight:700, cursor:'pointer' }}>Shopping Karo →</button>
    </div>
  );
  ...
  return (
    <div style={{ background:'#f5f9f6', minHeight:'80vh', fontFamily:"'Poppins',sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0a2e1a,#145530)', padding:'32px 36px' }}>
        <h1 style={{ fontFamily:"'Baloo 2',cursive", fontSize:32, fontWeight:800, color:'#fff', marginBottom:6 }}>Checkout 🛒</h1>
        <p style={{ color:'#86efac', fontSize:14 }}>Bas thoda aur — delivery details bharo!</p>
      </div>

      <div style={{ display:'grid', gridTemplateColumns:'1fr 380px', gap:24, padding:'28px 36px', maxWidth:1100, margin:'0 auto' }}>
        ...
        <div>
          ...
          <div style={{ background:'#fff', borderRadius:24, border:'1.5px solid #e8f2ea', padding:'28px', marginBottom:20 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 16px' }}>
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:'#6b8f71', display:'block', marginBottom:6 }}>Poora Naam *</label>
                <input style={inp} placeholder="Apna poora naam" value={address.name} onChange={e => setAddress({...address, name:e.target.value})} />
              </div>
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:'#6b8f71', display:'block', marginBottom:6 }}>Phone Number *</label>
                <input style={inp} placeholder="10 digit mobile number" value={address.phone} onChange={e => setAddress({...address, phone:e.target.value})} maxLength={10} />
              </div>
            </div>
            ...
            <input style={inp} placeholder="Paas mein mandir, school, hospital..." value={address.landmark} onChange={e => setAddress({...address, landmark:e.target.value})} />

            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 16px' }}>
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:'#6b8f71', display:'block', marginBottom:6 }}>Shahar / Town</label>
                <input style={inp} value={address.city} onChange={e => setAddress({...address, city:e.target.value})} />
              </div>
              <div>
                <label style={{ fontSize:12, fontWeight:600, color:'#6b8f71', display:'block', marginBottom:6 }}>Pincode</label>
            ...
            <button type="button" onClick={captureLocation}
              style={{ width:'100%', background:locationCaptured?'#e6f4ec':'#f5f9f6', color:locationCaptured?'#145530':'#4a6b50', border:`2px solid ${locationCaptured?'#b8dfc6':'#d0e8d8'}`, borderRadius:14, padding:'12px', fontSize:14, fontWeight:600, cursor:'pointer', fontFamily:"'Poppins',sans-serif", display:'flex', alignItems:'center', justifyContent:'center', gap:8, transition:'all 0.2s' }}>
              {locationCaptured ? '✅ Location Capture Ho Gayi!' : '📍 Apni Live Location Share Karo (Optional)'}
            </button>
          </div>
          ...
          <div style={{ background:'#fff', borderRadius:24, border:'1.5px solid #e8f2ea', padding:'28px' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
              {[
                ['cod','💵','Cash on Delivery','Delivery par cash dena hai'],
                ['online','📱','UPI / Card / Net Banking','Razorpay se secure payment'],
            ...
            {payMethod==='online' && RAZORPAY_KEY==='rzp_test_YOUR_KEY_HERE' && (
              <div style={{ background:'#fff7ed', borderRadius:14, padding:'12px 16px', marginTop:14, fontSize:13, color:'#f07c2a', fontWeight:600 }}>
                ⚠️ Razorpay key abhi set nahi hai. COD use karo ya Checkout.jsx mein key daalo.
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ background:'#fff', borderRadius:24, border:'1.5px solid #e8f2ea', padding:'24px', height:'fit-content', position:'sticky', top:80 }}>
          ...
          <div style={{ maxHeight:280, overflowY:'auto', marginBottom:16 }}>
            {cartItems.map(i => (
              <div key={i._id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 0', borderBottom:'1px solid #f0f7f2' }}>
                {i.img && <img src={i.img} alt={i.name} style={{ width:44, height:44, objectFit:'cover', borderRadius:12, flexShrink:0 }} />}
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'#0a2e1a' }}>{i.name}</div>
                  <div style={{ fontSize:11, color:'#6b8f71' }}>{i.quantity} × {i.qty}</div>
                </div>
                <div style={{ textAlign:'right' }}>
                  <div style={{ fontSize:14, fontWeight:800, color:'#145530', fontFamily:"'Baloo 2',cursive" }}>₹{i.price*i.qty}</div>
                  {i.mrp && i.mrp > i.price && <div style={{ fontSize:10, color:'#9ca3af', textDecoration:'line-through' }}>₹{i.mrp*i.qty}</div>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ paddingTop:4 }}>
            {savings > 0 && (
              <div style={{ background:'#e6f4ec', borderRadius:10, padding:'8px 12px', fontSize:12, color:'#145530', fontWeight:700, marginBottom:10, display:'flex', justifyContent:'space-between' }}>
                <span>🎉 Aapki Bachat</span><span>₹{savings}</span>
              </div>
            )}
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, marginBottom:8, padding:'6px 0' }}>
              <span style={{ color:'#6b8f71' }}>Delivery Charge</span>
              <span style={{ color:deliveryCharge===0?'#145530':'#0a2e1a', fontWeight:deliveryCharge===0?700:500 }}>
                {deliveryCharge===0?'FREE 🎉':`₹${deliveryCharge}`}
              </span>
            </div>
            {deliveryCharge>0 && (
              <div style={{ background:'#fff7ed', borderRadius:10, padding:'8px 12px', fontSize:12, color:'#f07c2a', fontWeight:600, marginBottom:8 }}>
                💡 ₹{199-total} aur add karo FREE delivery ke liye!
              </div>
            )}
